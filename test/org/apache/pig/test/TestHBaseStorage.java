...
public class TestHBaseStorage {
    ...
    @Test
    public void testDefaultColumnParsing1() throws IOException, ParseException {
      HBaseStorage storage = new HBaseStorage("foo:a foo:b");
      doColumnParseTest(storage);
    }

    public void doColumnParseTest(HBaseStorage storage) {
      Assert.assertEquals(2, storage.getColumnInfoList().size());
      byte[][] names = new byte[][] {Bytes.toBytes("a"), Bytes.toBytes("b")};

      for (byte[] name : names) {
        HBaseStorage.ColumnInfo col = storage.getColumnInfoList().remove(0);
        Assert.assertEquals(Byte.parseByte("foo"), col.getColumnFamily());
        Assert.assertEquals(name, col.getColumnName());
        Assert.assertNull(col.getColumnPrefix());
      }
    }
